...
function replydelete(replynum) {
	...
	if(answer) {
		location.href="replyDelete?replynum="+replynum+"&noticenum="+"${notice.noticenum}";
	}
	return;
}
function replymodify(replynum, text) {
	...
	document.getElementById("reply_submit").onclick = function() {
		if(document.getElementById("reply_submit").value == "댓글 수정") {
			...
			location.href="replyUpdate?replynum="+replynum+"&noticenum="+"${notice.noticenum}&text="+updatetext;
		}
	}
...
<table border="1">
	...
	<tr>
		...
		<c:if test="${mime == 'image'}">
			<img src="noticedownload?noticenum=${notice.noticenum}"/>
		</c:if>
		<c:if test="${notice.originalfile != null}">
			<a href="download?noticenum=${notice.noticenum}">${notice.originalfile}</a>
		</c:if>
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="2">
			<c:if test="${custid == notice.custid }">
			<input type="button" onclick="location.href='noticeUpdateForm?noticenum=${notice.noticenum}&currentPage=${currentPage}&searchtype=${searchtype}&searchword=${searchword}'" value="글수정" />
			<input type="button" onclick="location.href='noticeDelete?noticenum=${notice.noticenum}'" value="글삭제" />
			</c:if>
			<a href="noticeList?currentPage=${currentPage}&searchtype=${searchtype}&searchword=${searchword}">목록으로</a>
		</td>
	</tr>
</table>
<br />
...
<c:if test="${not empty custid }">
	<form id="replyWrite" action="notice" method="POST">
